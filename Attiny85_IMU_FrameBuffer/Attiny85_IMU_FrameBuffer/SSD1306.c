...
/***************************************************************************************************
 void OLED_DisplayPixel(uint8_t x, uint8_t y)
 ****************************************************************************************************
This function was made by Reed
 ****************************************************************************************************/
void OLED_DisplayPixel_SET(uint8_t x, uint8_t y){
// 	if (x>12){
// 		x = 0;
// 	}
// 	if (y>32){
// 		y = 0;
	...
	//add to buffer
	//int16_t buffer_index = 64*(y/8)+x;
	//buffer[buffer_index] |= (1<<(y%8));
	
	oledSendByte((1<<(y%8)));
	//oledSendByte(buffer[buffer_index]);
	oledSendStop();

}

void OLED_DisplayPixel_XOR(uint8_t x, uint8_t y){
	if (x>128){
		x = 0;
	}
	if (y>64){
	...
	//add to buffer
	uint16_t buffer_index = 128*(y/8)+x;
	buffer[buffer_index] ^= (1<<(y%8));
